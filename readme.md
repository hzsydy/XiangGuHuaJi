# 第14届~~香菇滑稽杯~~自动化系新生C语言大赛逻辑文档暨比赛章程
...
- 首都：首都是你的政治和经济中心（可以理解成司令部）。首都可以建立在军通区的任意位置，即可以放在同盟国境内。
...
- 首都是玩家指定的一个地点。如果玩家不指定，默认为上一回合的值。初始值被置为出生点。
  ...
  - 首都有一定的攻击力 ~~参考明代和清代将首都设在北京~~ 。只要你的首都本回合存在，就相当于在首都位置丢下等效于x的炸弹~~首都自带威慑力防御~~，x为你每回合收入的30%等效的炸弹规模，向上取整。
    - tips:在初期，利用频繁改变首都快速扩张领地是非常重要的~~红警里面拖着基地车到处跑~~
  - 首都除了有攻击力之外，还是你的经济中心，可以在经济阶段收钱（参见经济阶段）
  - 首都除了是你的经济和军事中心之外，还是你的政治中心。你的所有其他领地，必须要和你的首都，或者你的某个同盟国的首都，相连接，才能产生防御力。反之，则这些区域被包围(is sieged)，防御力降低为0。
    - ~~换句话说，你可以把每个首都都看作一个补给源，未能和任意一个友好的补给源相连的土地自然就断补(Out of Supply)了~~
    - 但是你依然可以投放兵力进一步组织进攻。~~强行连回来或者强行拆对面首都 或者如果胆子大你可以下一回合直接把首都搬过去~~ 

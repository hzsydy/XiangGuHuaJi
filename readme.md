...
- 领土变更的计算
  - 每单位 **有效** 的兵力对周围 **可以达到的点** 产生【被攻击格】攻击力\*一个随距离衰减的系数的攻击。相当于卷积一个高斯核然后再乘一个系数。
    - **可以达到的点** 默认只有一格。如果该格领土被 **击破** ，才能继续对后面的领土施加攻击力，这一行为称为 **连破** 。
    - **连破** 的实际实现方法为，卷积计算你可以击破的格子位置，并且舍去所有和你当前领土不联通的部分。
...
- 首都与 **包围**
  - 首都
    ...
    - 首都可以不设在自己领地而是设在有**军通**的同盟领地上。如果军通被取消，你将暂时（本回合）失去首都，你的所有领地视为被**包围** 。
    - 如果你的首都在本回合被攻陷，下回合开始时，你的全部地区视为被**包围** 。你可以指定一个新首都，但是得等到下回合结束的时候才能设立新首都。
  - 包围
    - 一种状态。在一回合结束时，所有没有 **军通** 到首都的领地视为被**包围** 。被**包围** 的地区的防御力视为0（但是依然可以投放兵力以进一步进攻）。
...
- 视野
  ...
  - 地图视野
    - 你知道每一块土地上的资源和攻防参数，但地图视野的限制使得你不能始终知道每一块土地的归属
...
- 国际关系总结
  ...
  | :-----: | :----------: | :-----: | :------------: | :----------------: | :---: |
  |         |     未发现      |   中立    |       停战       |         同盟         |  战争   |
  |   注释    |   还没有发现的国家   |  默认状态   | 等于中立，但x回合内不能宣战 | 自己对自己的国际关系始终标记为同盟。 |       |
  | 是否有侵犯效果 |      无       |    无    |       无        |         无          |   有   |
  | 是否有联防效果 |      无       |    无    |       无        |         有          |   无   |
